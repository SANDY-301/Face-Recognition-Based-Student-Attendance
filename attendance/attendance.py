...
def take_attendance(request):
    ...
    time_slots = {
        "morning": [
            ("1st_hour", 8, 45, 9, 10),
            ("2nd_hour", 9, 45, 10, 10),
            ("3rd_hour", 10, 45, 11, 10)
        ],
        "afternoon": [
            ("4th_hour", 12, 20, 12, 40),
            ("5th_hour", 13, 20, 13, 40)
        ]
    }
    ...
    for period, start_h, start_m, end_h, end_m in session_periods:
        if (start_h == current_hour and start_m <= current_minute) or (current_hour == end_h and current_minute <= end_m):
            status = "present"
    ...
    Attendance.objects.create(reg_no=recognized_student, session=session_requested, period=period_marked, status=status, date=today_date)
